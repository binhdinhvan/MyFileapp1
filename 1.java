...
public class MainActivity extends AppCompatActivity implements FileAdapter.OnItemClickListener {
    ...
    private void loadRecentFiles() {
        ...
        // Áp dụng kiểu sort người dùng chọn. Sort theo ngày -> vẫn nhóm theo ngày; theo tên/kích thước -> danh sách phẳng đã sort
        recent = FileListHelper.sort(recent, sortMode);
        List<FileItem> display;
        if (sortMode == SortMode.DATE_DESC || sortMode == SortMode.DATE_ASC) {
            display = groupFilesByDate(recent);
        } else {
            display = recent;
        }
    ...
    private void loadFiles(String path) {
        // Nếu đang ở tab Recent mà điều hướng vào folder (vd từ sidebar) -> quay lại chế độ Browse để hiện breadcrumb
        if (isRecentTab) {
            isRecentTab = false;
            adapter.setRecentMode(false);
            findViewById(R.id.breadcrumbScroll).setVisibility(View.VISIBLE);
            ...
            if (bn != null) bn.getMenu().findItem(R.id.nav_browse).setChecked(true);
        }
        currentPath = path;
        ...
        // Ẩn thanh tab Browser/Recent khi đang ở trong folder con, chỉ hiện ở thư mục gốc
        findViewById(R.id.bottomNavigation).setVisibility(path.equals(rootPath) ? View.VISIBLE : View.GONE);
    }
    ...
    private List<FileItem> groupFilesByDate(List<FileItem> original) {
        ...
        // Sắp xếp theo đúng sortMode hiện tại (tôn trọng Forward/Reverse) thay vì luôn theo ngày giảm dần
        filesToGroup = FileListHelper.sort(filesToGroup, sortMode);
